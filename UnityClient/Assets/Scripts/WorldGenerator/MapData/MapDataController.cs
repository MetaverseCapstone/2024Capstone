...
public class MapDataController : MonoBehaviour
{
	...
	public IEnumerator MapSave()
	{
		...
		int map_id = MapJsonFormat(mapData);
		int obj_cnt = ObjectJsonFormat(objectDatas, map_id);

		mapData.objCount = obj_cnt;

		// 맵 데이터 저장
		yield return mapToJson.SaveMapData((mapData, objectDatas));

		Debug.Log("MapSave");
	}

	// 맵 데이터의 다양한 속성을 설정하는 메서드
	public int MapJsonFormat(MapData mapData)
	{
		mapData.map_id = 0;
		mapData.mapName = "ExampleMap";
		mapData.mapCTime = Time.time;
	...
	public int ObjectJsonFormat(List<ObjectData> objectDatas, int map_id)
	{

		List<MapObject> objData = new List<MapObject>();
		...
		foreach (GameObject obj in FindObjectsOfType<GameObject>())
		{
			if (obj.CompareTag("Object"))
			{
				...
				Object.type = obj.tag;
				if (obj.GetComponentInChildren<Rigidbody>() != null) Object.rigidbody = true;
				if (obj.GetComponentInChildren<MeshCollider>() != null) Object.meshcollider = true;

				objData.Add(Object);
			}
		}
		if (objData.Count == 0) // 맵에 오브젝트가 하나도 존재하지 않은 경우
		{
			MapObject mapObject = new MapObject(); // Null 오브젝트 추가
			mapObject.type = "Null";
			objData.Add(mapObject);
		}

		ObjectData objectData = new ObjectData();
		objectData.map_id = map_id;
		for (int i = 0; i < objData.Count; i++)
		{
			if ((i + 1) % 15 == 0)
			{
				objectData.objects.Add(objData[i]);
	...
	{
		yield return StartCoroutine(mapToJson.LoadMapData()); // 저장된 맵 데이터 불러오기

		if (mapToJson.GetLoadSuccess())
		{
			(MapData, List<ObjectData>) MapDataTuple = mapToJson.GetMapData();
	...
	// 저장된 오브젝트를 로드하는 메서드
	public IEnumerator ObjectLoad(List<ObjectData> MapDataObjects)
	{
		...
		foreach (ObjectData MapDataObject in MapDataObjects)
		{
			foreach (MapObject mapObject in MapDataObject.objects)
			{
